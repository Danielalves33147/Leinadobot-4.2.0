...
!ia <pergunta> — Consulta a IA para respostas`;
          ...
          case '!all':
            try {
              ...
              const mentionsFiltradas = mentions.filter(id => !botIds.includes(id));

              const texto = '📍Chamando todo mundo📍';

              await sock.sendMessage(jid, { text: texto, mentions: mentionsFiltradas });
          ...
          case '!sorteio':
            try {
              ...
              // remove o próprio bot (por garantia, mesmo se mudar de ID)
              const participantesValidos = participantes.filter(id => id !== sock.user.id);

              if (participantesValidos.length === 0) {
                await reply({ text: '⚠️ Não há participantes válidos neste grupo para sortear.' });
                break;
              }

              if (numSorteados > participantesValidos.length) {
                await reply({ text: '⚠️ O número de vencedores é maior que o número de participantes.' });
                break;
              }

              const participantesRestantes = [...participantesValidos]; // cópia segura
              const vencedores = [];
              ...
              // menciona só os vencedores, sem risco de incluir o bot
...
Aproveite o poder do LeinadoBot!`;
          ...
          case '!perdi':
            try {
              ...
              // JIDs a serem mencionados "invisivelmente"
              const mentions = [
              ...
              // Texto limpo (sem @ no corpo)
              const texto = `Perdemos *${currentCount}* vez(es)... 😔`;

              // Menciona igual ao !all: metadados em "mentions", sem expor @ no texto
    ...
    `, [senderJid, null, '!cargo']);
          ...
          case '!addcargo':
            try {
              ...
              // Sanitiza o @usuario (só dígitos)
              const targetUserIdAdd = args[0].replace(/[^0-9]/g, '') + '@s.whatsapp.net';

              // Junta o resto como nome do cargo (permite cargos com espaço) e normaliza
              const cargoEntradaRaw = args.slice(1).join(' ');
              const cargoEntradaNorm = normalizeNome(cargoEntradaRaw);
              ...
              // Carrega cargos e compara por nome normalizado (sem acento e minúsculo)
              const cargosRes = await dbClient.query('SELECT id, nome FROM cargos');
              const match = cargosRes.rows.find(row => normalizeNome(row.nome) === cargoEntradaNorm);

              if (!match) {
                const disponiveis = cargosRes.rows.map(r => r.nome).join(', ');
                await reply({ text: `⚠️ Cargo não encontrado: *${cargoEntradaRaw}*.\nDisponíveis: ${disponiveis}` });
                break;
              }

              const novoCargoId = match.id;           // id do cargo no banco
              const cargoNomeCanonico = match.nome;   // nome exatamente como está no banco
              ...
              // Hierarquia: precisa ser >= do cargo que está dando e > do cargo atual do alvo
              if (
              ...
              await reply({ text: `✅ Cargo *${cargoNomeCanonico}* atribuído a @${targetUserIdAdd.split('@')[0]}.`, mentions: [targetUserIdAdd] });
    ...
    `, [senderJid, targetUserId, '!removecargo']);
          ...
          case '!listarcargos':
            try {
              ...
              // remove o próprio bot da lista e da menção
              usuariosFiltrados = usuariosFiltrados.filter(row => row.user_id !== botId);
              ...
              // Texto com @ visível, mas sem marcar o bot
    ...
    `, [senderJid, conteudoRaw || null, `!mass ${tipoAcao}`]);
          ...
          case '!fazol':
            try {
              ...
              // JIDs a mencionar "por baixo dos panos"
              const mentions = [
                '557191165170@s.whatsapp.net', // Daniel
                '557182903278@s.whatsapp.net', // Melky
                '557199670849@s.whatsapp.net', // Michael
              ...
              // Envia como o !all: texto limpo + mentions nos metadados
              await sock.sendMessage(jid, { text: texto, mentions });

              // Log (opcional)
              await dbClient.query(
                `INSERT INTO logs (user_id, alvo_id, comando) VALUES ($1, $2, $3)`,
